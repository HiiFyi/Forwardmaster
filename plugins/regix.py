...
async def pub_(bot, message):
    ...
    await send(client, user, "<b>Fᴏʀᴡᴀᴅɪɴɢ sᴛᴀʀᴛᴇᴅ. ᴘᴏᴡᴇʀᴇᴅ ʙʏ: @botmaster55 🔥</b>")
    sts.add(time=True)
    ...
    if locked:
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
            ...
            return await stop(client, user)
        temp.IS_FRWD_CHAT.remove(sts.TO)
        await send(client, user, "<b>🎉 ғᴏʀᴡᴀᴅɪɴɢ ᴄᴏᴍᴘʟᴇᴛᴇᴅ. ᴘᴏᴡᴇʀᴇᴅ ʙʏ: @botmaster55 🔥</b>")
...
async def edit(user, msg, title, status, sts):
   ...
   if status in ["cancelled", "completed"]:
      button.append([InlineKeyboardButton('• ᴄᴏᴍᴘʟᴇᴛᴇᴅ ​•', url='https://t.me/botmaster55')])
   else:
